## KiCad Schematic Representation in PCBRev

The application uses a comprehensive object-oriented Dart representation of KiCad schematics that closely mirrors the KiCad S-expression file format structure. This document reflects the implementation in `lib/features/symbol_library/data/kicad_schematic_models.dart`.

Note that while other parts of the application use `record` types for data modeling, the `symbol_library` feature uses `class`es for a more direct mapping of the KiCad file format.
...
### Core Schematic Model (KiCadSchematic)
...
#### 1. SymbolInstance (Component Placement)
...
*Note: `Position`, `Property`, `Stroke`, and `TextEffects` are defined in `kicad_symbol_models.dart`. `Size` is from `dart:ui`.*
